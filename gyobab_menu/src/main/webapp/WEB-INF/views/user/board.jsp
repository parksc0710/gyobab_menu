	            ...
	            <div class="card mb-3">
					<div class="card-header">
				        ...
				        <span class="pull-right">
				    		...
				    		<a href="${pageContext.request.contextPath}/user/board.do" type="button" class="btn btn-outline-primary mb-2 active">작성 글</a>
							  ...
							  <tbody>
							  	...
							  	<c:forEach var="list" items="${list}">
							  		<tr>
							  			<td>
							  			<c:choose>
											<c:when test="${list.board_type eq 'notice'}">
												공지사항
									  		</c:when>
									  		<c:when test="${list.board_type eq 'menu' }">
												오늘의 점심 메뉴
									  		</c:when>
									  		<c:when test="${list.board_type eq 'restaurant' }">
									  			주변 식당 정보
									  		</c:when>
									  		<c:when test="${list.board_type eq 'humor' }">
									  			유머게시판
									  		</c:when>
									  		<c:when test="${list.board_type eq 'free' }">
									  			자유게시판
									  		</c:when>
									  	</c:choose>
							  			</td>
...
<script>
	...
	function jsMakePage(pCurPage, totCount, maxSize) {
		...
		if (nPageCount > nPageRange) {
			...
			retVal += '<a href="${pageContext.request.contextPath}/user/board.do" aria-controls="dataTable" data-dt-idx="0" tabindex="0" class="page-link"><<</a></li>';
		}
		
		if (nPrev > 0) {
			retVal += '<li class="paginate_button page-item previous" id="dataTable_previous">';
			retVal += '<a href="${pageContext.request.contextPath}/user/board.do?pageNum='+(nPrev+3)+'" aria-controls="dataTable" data-dt-idx="0" tabindex="0" class="page-link"><</a></li>';
		} else {
			retVal = "&nbsp;";
		}
		...
		do {
			if (pCurPage == nCur) {
				...
				retVal += 	'<a href="${pageContext.request.contextPath}/user/board.do?pageNum='+nCur+'" aria-controls="dataTable" data-dt-idx="'+nCur+'" tabindex="0" class="page-link">'+nCur+'</a>';
				retVal += '</li>';
			} else {
		...
		if (nNext <= nPageCount) {
			retVal += '<li class="paginate_button page-item previous" id="dataTable_previous">';
			retVal += '<a href="${pageContext.request.contextPath}/user/board.do?pageNum='+nNext+'" aria-controls="dataTable" data-dt-idx="0" tabindex="0" class="page-link">></a></li>';
			
			retVal += '<li class="paginate_button page-item previous" id="dataTable_previous">';
			retVal += '<a href="${pageContext.request.contextPath}/user/board.do?pageNum='+nPageCount+'" aria-controls="dataTable" data-dt-idx="0" tabindex="0" class="page-link">>></a></li>';
